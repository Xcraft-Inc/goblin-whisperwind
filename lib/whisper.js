...
const {createAudio} = require('./utils.js');
...
class Whisper extends Elf {
  ...
  async configure(enableTermuxLogger = false) {
    if (enableTermuxLogger) {
      ...
      this._logger = async (msg) => {
        this.quest.evt('<whisper-output>', {id: this.state.id, output: msg});
      };
    }
  ...
  async start(loopback = false, model, inputName, language) {
    if (!inputName) {
      const usableDevice = await this.updateUsableDevices();
      const device = usableDevice.find(
        (d) => d.deviceIndex === this.state.defaultDeviceId
      );
    ...
    if (this.state.mode === 'local') {
      ...
      await this._whisperThread.init(modelFilePath, this.state.mode, language);
    } else {
      await this._whisperThread.init(null, this.state.mode, language);
    }

    this._abortController = new AbortController();
    try {
      this.log.dbg('Start recording on device :', inputName);
  ...
  async getAudioDevices() {
    ...
    const devices = this.audio.getDevices();
    return devices;
  }
