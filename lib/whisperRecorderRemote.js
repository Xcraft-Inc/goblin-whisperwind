...
const {resampleTo16kHz, createAudio} = require('./utils.js');
...
class WhisperRecorderRemote {
  ...
  constructor(options = {}, logger) {
    ...
    this.FRAME_SIZE = options.frameSize || 1920; // audio frame to 48k → ~40ms
   ...
   * @param {number} inputName - Audio device name (prefix).
   */
  async start(sessionId, inputName) {
    this.getDevices();
    const device = this.devices.reduce((input, device) => {
      if (device.name.startsWith(inputName) && device.inputChannels > 0) {
        input = device;
      }
      return input;
    }, this.devices[0]);
    ...
    const inputSampleRate = device.sampleRate || 48000;
    ...
    this.audio.openInputStream(
      device.id,
      1,
      this.audio.SampleFormat.FLOAT32,
      inputSampleRate,
      frameSize,
      async (pcmBuffer) => {
        if (!this.isRunning) {
